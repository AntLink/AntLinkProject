...
# start navbar tag
@register.inclusion_tag('layout/navbar_list.html')
...
def tag_widget(data, position=None):
    v = json.loads(data)
    type = v['taxonomy']
    tax = Taxonomy.objects.filter(type=type).order_by('?')[:30]
    return {
        'taxs': tax,
        'display': v['display'],
...
def category_widget(data, position=None):
    v = json.loads(data)
    if (v['display'] == 'tree'):
        tax = Taxonomy.objects.filter(type='category')
    else:
        tax = Taxonomy.objects.filter(type='category').order_by('?')[:25]
...
@register.inclusion_tag('prontend/home_slider.html')
def home_slider():
...
@register.inclusion_tag("prontend/images_gallery.html")
